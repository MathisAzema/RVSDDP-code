...
@everywhere function rvsddp_periodic_job(seed, parallel, time_max, shift_function, discount_factor, refine_scheme)
    ...
    Random.seed!(seed)
    Cuts=RVSDDP.train(model; refine_scheme=refine_scheme, parallel=parallel, sampling_scheme=RVSDDP.InSampleMonteCarlo(rollout_limit = i -> 1*10-1), time_limit = time_max, infinite = true, shift_function=shift_function); 
