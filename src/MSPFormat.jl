...
function _parse_lattice(filename::String)
    ...
    for (key, value) in data
        ...
        for child in sort!(stage_node_combinations[value["stage"]+1])
            ...
            RVSDDP.add_edge(graph, key => child, prob)
        end
    end
    ...
    for key in stage_zero
        RVSDDP.add_edge(graph, "root" => key, 1 / length(stage_zero))
    end
...
function _reduce_lattice(graph, data)
    ...
    new_graph = RVSDDP.Graph("root")
    for t in keys(nodes_by_stage)
        RVSDDP.add_node(new_graph, "$t")
    end
    for t in keys(nodes_by_stage)
        if t == 0
            RVSDDP.add_edge(new_graph, "root" => "$t", 1.0)
        else
            RVSDDP.add_edge(new_graph, "$(t-1)" => "$t", 1.0)
        end
    end
    graph_data = Dict(
        "$t" => Dict{String,Any}(
    ...
    for term in terms
        ...
        if x isa JuMP.VariableRef
            ...
            elseif term["stage"] == stage - 1
                ...
                error(
                    "RVSDDP.jl does not support this MSPFormat file because it " *
...
Return a [`RVSDDP.PolicyGraph`](@ref) built from the MSPFormat files
...
function read_from_file(
    ...
    model = RVSDDP.PolicyGraph(
        ...
        RVSDDP.parameterize(sp, Ω, P) do ω
            RVSDDP.@stageobjective(
                sp,
                stage_objective + sum(
