...
Run RVSDDP in asynchronous mode workers with pid's `slave_pids`.
...
Run RVSDDP in multi-threaded mode.
...
!!! danger
    This plug-in is experimental, and parts of RVSDDP.jl may not be threadsafe. If
    you encounter any problems or crashes, please open a GitHub issue.

## Example

```julia
RVSDDP.train(model; parallel_scheme = RVSDDP.Threaded())
RVSDDP.simulate(model; parallel_scheme = RVSDDP.Threaded())
```
"""
