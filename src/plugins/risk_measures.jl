...
```jldoctest
julia> RVSDDP.ConvexCombination(
           (0.5, RVSDDP.Expectation()),
           (0.5, RVSDDP.AVaR(0.25))
       )
A convex combination of 0.5 * RVSDDP.Expectation() + 0.5 * RVSDDP.AVaR(0.25)
```
...
```jldoctest
julia> 0.5 * RVSDDP.Expectation() + 0.5 * RVSDDP.AVaR(0.5)
A convex combination of 0.5 * RVSDDP.Expectation() + 0.5 * RVSDDP.AVaR(0.5)
```
"""
...
```jldoctest
julia> RVSDDP.EAVaR(; lambda = 1.0, beta = 1.0)
A convex combination of 1.0 * RVSDDP.Expectation() + 0.0 * RVSDDP.AVaR(1.0)

julia> RVSDDP.EAVaR(; lambda = 0.0, beta = 1.0)
A convex combination of 0.0 * RVSDDP.Expectation() + 1.0 * RVSDDP.AVaR(1.0)

julia> RVSDDP.EAVaR(; lambda = 0.5, beta = 0.5)
A convex combination of 0.5 * RVSDDP.Expectation() + 0.5 * RVSDDP.AVaR(0.5)
```
"""
...
The distributionally robust RVSDDP risk measure of
Philpott, A., de Matos, V., Kapelevich, L. Distributionally robust RVSDDP.
...
julia> RVSDDP.adjust_probability(
           ...
           RVSDDP.Noise.([1, 2, 3, 4], [0.25, 0.25, 0.25, 0.25]),  # noise_supports,
           [5.0, 4.0, 6.0, 2.0],  # cost_realizations,
           true,                  # is_minimization
       )
0.0
...
```julia
wasserstein_norm(x::RVSDDP.Noise, y::RVSDDP.Noise)::Float64
```
...
julia> wasserstein_norm(x::RVSDDP.Noise, y::RVSDDP.Noise) = abs(x.term - y.term);

julia> RVSDDP.adjust_probability(
           RVSDDP.Wasserstein(wasserstein_norm, HiGHS.Optimizer; alpha = 0.5),
           risk_adjusted_probability,
           [0.1, 0.2, 0.3, 0.4],  # nominal_probability,
           RVSDDP.Noise.([1.0, 2.0, 3.0, 4.0], [0.1, 0.2, 0.3, 0.4]),  # noise_supports,
           [5.0, 4.0, 6.0, 2.0],  # cost_realizations,
           true,                  # is_minimization
       )
0.0
...
Base.show(io::IO, measure::Wasserstein) = print(io, "RVSDDP.Wasserstein")
