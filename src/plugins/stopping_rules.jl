    ...
    function Statistical(;
        ...
        if !disable_warning
            @warn(
                ...
                "RVSDDP.Statistical` into the REPL to read the docstring.\n\n" *
                "If you understand what you are doing, you can disable this " *
                "warning with `RVSDDP.Statistical(; disable_warning = true)`",
            )
        end
...
"""
    SimulationStoppingRule(;
        sampling_scheme::AbstractSamplingScheme = RVSDDP.InSampleMonteCarlo(),
        replications::Int = -1,
        period::Int = -1,
...
By default, `replications` and `period` are `-1`, and RVSDDP.jl will guess good
...
```julia
RVSDDP.train(model; stopping_rules = [SimulationStoppingRule()])
```
"""
...
```julia
RVSDDP.train(model; stopping_rules = [FirstStageStoppingRule()])
```
"""
