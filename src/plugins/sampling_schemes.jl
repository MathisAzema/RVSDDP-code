...
a vector of new [`RVSDDP.Noise`](@ref) terms for the children of that node, and
a vector of new [`RVSDDP.Noise`](@ref) terms for the stagewise-independent
noise.
...
[`RVSDDP.Noise`](@ref) for the children of the root node.
...
If a node is deterministic, pass `[RVSDDP.Noise(nothing, 1.0)]` as the vector of
noise terms.
...
sampler = OutOfSampleMonteCarlo(graph) do node
    if node == 0
        return [RVSDDP.Noise(1, 1.0)]
    else
        noise_terms = [RVSDDP.Noise(node, 0.3), RVSDDP.Noise(node + 1, 0.7)]
        children = node < T ? [RVSDDP.Noise(node + 1, 0.9)] : RVSDDP.Noise{Int}[]
...
sampler = OutOfSampleMonteCarlo(graph, use_insample_transition=true) do node
    return [RVSDDP.Noise(node, 0.3), RVSDDP.Noise(node + 1, 0.7)]
end
```
"""
...
The sample space for [`RVSDDP.parameterize`](@ref) must be a tuple with 1 or 2
...
julia> model = RVSDDP.PolicyGraph(
           RVSDDP.MarkovianGraph(simulator; budget = 20, scenarios = 100);
           sense = :Max,
           upper_bound = 12,
           optimizer = HiGHS.Optimizer,
       ) do sp, node
           t, markov_state = node
           @variable(sp, x >= 0, RVSDDP.State, initial_value = 1)
           @variable(sp, u >= 0)
           ...
           RVSDDP.parameterize(sp, Ω) do (markov_state, ω)
...
julia> RVSDDP.train(
           ...
           print_level = 0,
           iteration_limit = 10,
           sampling_scheme = RVSDDP.SimulatorSamplingScheme(simulator),
       )
