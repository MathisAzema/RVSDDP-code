...
function _deprecate_integrality_handler()
    return error(
        """
RVSDDP.jl v0.4.0 introduced a number of breaking changes in how we deal with
 ...
 * Instead of passing the argument to `PolicyGraph`, you now pass it to
   `train`, e.g., `RVSDDP.train(model; duality_handler = RVSDDP.LagrangianDuality())`
...
`RVSDDP.StrengthenedConicDuality()`.
...
please reach out or read https://github.com/odow/RVSDDP.jl/issues/246.
...
julia> handler = RVSDDP.LagrangianDuality(Ipopt.Optimizer)
RVSDDP.LagrangianDuality{DataType}(RVSDDP.LocalImprovementSearch.BFGS(100), Ipopt.Optimizer)
```
...
julia> handler = RVSDDP.BanditDuality(Ipopt.Optimizer)
BanditDuality with arms:
 * RVSDDP.ContinuousConicDuality{DataType}(Ipopt.Optimizer)
 * RVSDDP.StrengthenedConicDuality{DataType}(Ipopt.Optimizer)
```
"""
