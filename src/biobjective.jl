...
This must be called from inside [`RVSDDP.parameterize`](@ref).
...
function set_biobjective_functions(subproblem, objective_1, objective_2)
    λ = RVSDDP.objective_state(subproblem)
...
This must be called outside [`RVSDDP.parameterize`](@ref).

!!! warning
    This function is experimental! It may change in any future release.
"""
function initialize_biobjective_subproblem(subproblem)
    RVSDDP.add_objective_state(
        subproblem;
        initial_value = 0.0,
        lower_bound = 0.0,
        upper_bound = 1.0,
...
"""
    set_trade_off_weight(model::RVSDDP.PolicyGraph, weight::Float64)
...
!!! warning
    This function is experimental! It may change in any future release.
"""
function set_trade_off_weight(model::RVSDDP.PolicyGraph, weight::Float64)
    @assert 0 <= weight <= 1
 ...
 * `kwargs` are passed to [`RVSDDP.train`](@ref) when solving the scalarized
   problems.
...
function train_biobjective(
    ...
    solution_limit::Int,
    include_timing::Bool = false,
    log_file_prefix::String = "RVSDDP",
    stopping_rules::Function = weight -> RVSDDP.AbstractStoppingRule[],
    kwargs...,
)
    start_time = time()
    ...
    for weight in (0.0, 1.0)
        ...
        solutions[weight] = value(RVSDDP.calculate_bound(model))
    end
    ...
    while length(queue) > 0 && length(solutions) < solution_limit
        ...
        bound = RVSDDP.calculate_bound(model)
